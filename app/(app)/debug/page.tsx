...
export default async function DebugPage() {
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'monospace', fontSize: '12px' }}>
      ...
      {userError && <pre style={{ color: 'red' }}>Error: {JSON.stringify(userError, null, 2)}</pre>}
      
      <h2>All Teams in Database</h2>
      <pre>{JSON.stringify(allTeams, null, 2)}</pre>
      {teamsError && <pre style={{ color: 'red' }}>Error: {JSON.stringify(teamsError, null, 2)}</pre>}
      
      <h2>My Memberships (raw query)</h2>
      <pre>{JSON.stringify(myMemberships, null, 2)}</pre>
      {membershipsError && <pre style={{ color: 'red' }}>Error: {JSON.stringify(membershipsError, null, 2)}</pre>}
