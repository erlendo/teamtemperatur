-- Create a debug version of get_team_year_stats without RLS checks
create or replace function public.debug_get_team_year_stats(
  p_team_id uuid
)
returns table (
  week int,
...
-- Now run the debug query
SELECT 
  week,
  response_count,
  member_count,
  response_rate,
  jsonb_array_length(question_stats) as num_questions
FROM public.debug_get_team_year_stats('dbbd1841-eee9-4091-968e-69b8b6214b8e'::uuid)
WHERE week = 6;
