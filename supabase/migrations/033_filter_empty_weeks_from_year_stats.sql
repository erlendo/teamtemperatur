-- Filter out weeks with no submissions from year stats
-- Only return weeks that have actual responses
...
begin
  ...
  bayesian_calc as (
    ...
    from weeks_with_data wd
    cross join active_members am
    left join weekly_submissions wsub on wsub.week = wd.wk
    left join question_stats qs on qs.week = wd.wk
  ),
  moving_avg_calc as (
    select
      bc.week,
